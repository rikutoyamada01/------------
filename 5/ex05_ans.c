...
int main() {
    ...
    printf("設問５：読み込んだデータ，合計点・平均点をoutput.csvに書き込み，確認のため合計点を出力\n");
    // 設問５：設問５：読み込んだデータ，合計点・平均点をoutput.csvに書き込み，確認のため合計点・平均点を出力
    fp = fopen("output.csv", "w"); // ファイルオープン
    fprintf(fp, "国語,数学,英語,理科,社会,合計,平均\n"); // csvのヘッダーを書き込み
    for(i=0; i<N1; i++) {
        int total = 0;
        float ave = 0;
        for (j=0; j<N2; j++) {
...
p[9][3]:96
p[9][4]:77
設問５：読み込んだデータ，合計点・平均点をoutput.csvに書き込み，確認のため合計点を出力
p[0] total=228
p[0] ave=45.60
p[1] total=228
p[1] ave=45.60
